...
LmcpObjectNetworkBridgeManager::createBridge(const pugi::xml_node& bridgeXmlNode, uint32_t entityId, uint32_t networkId)
{
    ...
    if (uxas::common::StringConstant::Bridge().compare(bridgeXmlNode.name()) == 0)
    {
        ...
        else if (LmcpObjectNetworkCamkesMultiReceiverBridge::s_typeName().compare(bridgeType) == 0)
        {
            newBridge = uxas::stduxas::make_unique<LmcpObjectNetworkCamkesMultiReceiverBridge>();
        }
        else if (LmcpObjectNetworkCamkesReceiverBridge::s_typeName().compare(bridgeType) == 0)
        {
            newBridge = uxas::stduxas::make_unique<LmcpObjectNetworkCamkesReceiverBridge>();
        }
        else if (LmcpObjectNetworkCamkesTransmitterBridge::s_typeName().compare(bridgeType) == 0)
        {
            newBridge = uxas::stduxas::make_unique<LmcpObjectNetworkCamkesTransmitterBridge>();
        }
