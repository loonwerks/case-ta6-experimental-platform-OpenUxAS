    ...
    namespace lmcp {
        ...
        Object * Factory::createObject(int64_t series_id, uint32_t type, uint16_t version)
        {
            if (series_id == 6149751333668345413LL)
               return uxas::messages::uxnative::UXNATIVEFactory::createObject(series_id, type, version);
            if (series_id == 6149757930721443840LL)
               return uxas::messages::task::UXTASKFactory::createObject(series_id, type, version);
            if (series_id == 5931053054693474304LL)
               return uxas::messages::route::ROUTEFactory::createObject(series_id, type, version);
            if (series_id == 5784119745305990725LL)
               return afrl::cmasi::perceive::PERCEIVEFactory::createObject(series_id, type, version);
            if (series_id == 6216454340153722195LL)
               return afrl::vehicles::VEHICLESFactory::createObject(series_id, type, version);
            if (series_id == 5281966179208134656LL)
               return afrl::impact::IMPACTFactory::createObject(series_id, type, version);
            if (series_id == 4849604199710720000LL)
               return afrl::cmasi::CMASIFactory::createObject(series_id, type, version);
